...
const LANGS = [
    ...
    { flag: '🇹🇷', code: 'tr', label: 'TR' },
    { flag: '🇷🇺', code: 'ru', label: 'RU' },
    { flag: '🇩🇪', code: 'de', label: 'DE' },
]
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
                    ...
                    <div className="flex flex-col gap-6">
                        ...
                        <div className="flex items-center gap-2 w-fit flex-wrap">
                            <span className="flex items-center gap-1.5 bg-[#1a6b4a]/10 text-[#1a6b4a] px-3 py-1.5 rounded-full text-xs font-semibold tracking-wide border border-[#1a6b4a]/15">
                        ...
                        <div className="flex items-center gap-4 flex-wrap">
                            <Link
                                to="/products"
                                search={{category:'all'}}
                                className="inline-flex items-center gap-2 bg-[#1a6b4a] text-white px-6 py-3.5 rounded-xl text-sm font-semibold shadow-[0_4px_16px_rgba(26,107,74,0.35)] hover:bg-[#165c3f] hover:shadow-[0_8px_24px_rgba(26,107,74,0.45)] hover:-translate-y-0.5 transition-all duration-200"
                            >
                                {t.hero.cta}
                            ...
                            <a
                                ...
                                className="inline-flex items-center gap-2 bg-white/80 text-[#1a6b4a] px-6 py-3.5 rounded-xl text-sm font-semibold border border-[#c8e6d4] hover:bg-white hover:border-[#1a6b4a]/30 transition-all duration-200"
                            >
                            ...
                            {t.features.map((f) => {
                                ...
                                return (
                                    <div key={f.key} className="relative">
                                        <button
                                            onClick={handleClick}
                                            className="w-full flex items-center gap-3 bg-white/60 backdrop-blur-sm rounded-xl px-4 py-3 border border-[#c8e6d4]/50 hover:border-[#1a6b4a]/40 hover:bg-white/80 transition-all duration-200 cursor-pointer text-left"
                                        >
                                        ...
                                        {/* Inline lang picker */}
                                        {isLang && showLangPicker && (
                                            <div className="absolute top-full left-0 right-0 mt-1 bg-white rounded-xl border border-[#c8e6d4] shadow-lg p-2 z-20 grid grid-cols-2 gap-1">
                                                {LANGS.map(l => (
                                                    <button
                                                        key={l.code}
                    ...
                    <div className="hidden lg:flex items-center justify-center">
                        <div className="relative">
                            ...
                            {/* Main card */}
                            <div className="bg-white rounded-3xl shadow-[0_24px_64px_rgba(26,107,74,0.12)] border border-[#c8e6d4]/60 p-8 w-[340px]">
                                ...
                                <div className="grid grid-cols-4 gap-2 mb-6">
                                    {CATEGORY_KEYS.map((key) => {
                                        ...
                                        return (
                                            <button
                                                key={key}
                                                onClick={() => navigate({ to: '/products', search: { category: key } })}
                                                className="relative group aspect-square rounded-xl flex items-center justify-center transition-all duration-200 cursor-pointer hover:scale-105 hover:shadow-md"
                                                style={{ backgroundColor: cat?.bg }}
                                                title={t.categories[key]}
                                            >
                                                {cat && <cat.icon className="w-5 h-5" style={{ color: cat.color }} />}
                                                <div className="absolute -top-8 left-1/2 -translate-x-1/2 bg-[#0f2d1f] text-white text-[10px] font-medium whitespace-nowrap px-2 py-1 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none z-10">
                                                    {t.categories[key]}
                                                </div>
                                            </button>
                                        )
                                ...
                                <div className="flex gap-2 flex-wrap mb-5">
                                    {LANGS.map(l => (
                                        <button
                                            ...
                                            className={[
                                                'text-xs font-semibold px-2.5 py-1 rounded-lg border transition-all duration-200 cursor-pointer',
                                                lang === l.code
                                                    ? 'bg-[#1a6b4a] text-white border-[#1a6b4a]'
                                                    : 'bg-[#1a6b4a]/8 text-[#1a6b4a] border-[#1a6b4a]/15 hover:bg-[#1a6b4a]/15',
                                            ].join(' ')}
                                        >
                                            {l.flag} {l.label}
                                        </button>
                            ...
                            <div className="absolute -top-4 -right-4 bg-white rounded-2xl px-4 py-2.5 shadow-[0_8px_24px_rgba(26,107,74,0.12)] border border-[#c8e6d4]/60 flex items-center gap-2">
                                <MapPin className="w-4 h-4 text-[#1a6b4a]" />
                                <span className="text-xs font-semibold text-[#0f2d1f]">Belek, Antalya</span>
                            </div>

                            {/* Floating pill bottom left */}
                            <div className="absolute -bottom-4 -left-4 bg-[#1a6b4a] rounded-2xl px-4 py-2.5 shadow-[0_8px_24px_rgba(26,107,74,0.3)] flex items-center gap-2">
